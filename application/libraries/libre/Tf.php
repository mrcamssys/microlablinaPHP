...
class Tf{
        ...
        public function __construct($num=0,$den=0,$ident="H(s)",$msg="L.Abierto") {
          $num1=array();
          $den1=array();  
        ...
        public function getfracpar(){
            ...
            {   
             ...
             $maxi=$maxi-1;
            $texto['Fparciales'] = $texto['Fparciales'].'<div class="alert alert-info" role="alert">Recuerde que para encontrar la función transferencia equivalente el en mundo de la frecuencia debe aplicar: $$ H(s)=\sum_{k=0}^{'.$maxi.'} H_k(S) $$ </div>';


            $texto['inversaplace'] = $texto['inversaplace'].'<div class="alert alert-info" role="alert">Recuerde que para encontrar la solución en el tiempo del sistema debe aplicar: $$ h(t)=\sum_{k=0}^{'.$maxi.'} h_k(t) $$ </div>';
            return array($texto['Fparciales'],$texto['inversaplace']);
        }
        
        public function getbode(){
            $sys=new Bode($this->num->polinomio,$this->den->polinomio);
            $fase=$sys->fase();
            $bode=$sys->magnitud();
