...
class Tfballbeam extends CI_Controller {
    ...
    private function librerias()
    {
       ...
       $this->load->library('libre/balbeam');
       // $this->librerias();
    }
/*
...
*/
    public function pidballbeam(){
      $this->librerias();
      $ballbeam= new Balbeam();
        $this->texto['menux']=new Menu1($this->menu,"PID posición");
        
        
        if($this->input->post("enviado")!=1){
          $this->texto['ball_beta']=0;
          $this->texto['ball_gamma']=0;
          $this->texto['ball_alpha']=0;
          $this->texto['ball_d']=0;
          $this->texto['ball_m']=0;
          $this->texto['ball_j']=0;
          $this->texto['ball_r']=0;
          ...
          $this->texto['ki']=0;
          $this->texto['kd']=0;
          $this->texto['ball_num']=0;
          $this->texto['ball_den']=0;
        }else{
          $beta=floatval($this->input->post("ball_beta"));
          $gamma=floatval($this->input->post("ball_gamma"));
          $alpha=floatval($this->input->post("ball_alpha"));
          $d=floatval($this->input->post("ball_d"));
          $m=floatval($this->input->post("ball_m"));
          $j= floatval($this->input->post("ball_j"));
          $r=floatval($this->input->post("ball_r"));
          
          $ballbeam->nuevosParametros($beta,$gamma,$alpha,$d,$m,$j,$r);
          ...
          $this->texto['ball_beta']=$ballbeam->beta;
          $this->texto['ball_gamma']=$ballbeam->gamma;
          $this->texto['ball_alpha']=$ballbeam->alpha;
          $this->texto['ball_d']=$ballbeam->d;
          $this->texto['ball_m']=$ballbeam->m;
          $this->texto['ball_j']=$ballbeam->j;
          $this->texto['ball_r']=$ballbeam->r;
          ...
          $this->texto['ps']=$data;
          list($this->texto['ball_num'],$this->texto['ball_den'])=$ballbeam->tf2tbox($data);
        }
        ...
        if(isset($this->session->email)){       
            ...
            $this->load->view('modelosTF/ballbeamtp',$this->texto);
    ...
    public function __construct(){
         ...
         parent::__construct();
           $this->menu=[
            base_url()."tfballbeam/pidballbeam"=>"PID posición",
            //base_url()."tfballbeam/tfmotorVE"=>"Representacion en Variables Estado",
            base_url()."tfballbeam/teoria"=>"Teoria del Sistema",
            ]; 
    }
 ...
 public function index(){
    ...
    if(isset($this->session->email)){       
        $this->load->view('logeado/head1');  
        $this->load->view('logeado/cabecera1');
        $this->load->view('modelosTF/ballbeam',$this->texto);
        $this->load->view('logeado/ehtml');  
        if($this->input->get('logout')=='ok'){
            $this->session->sess_destroy();
            redirect(base_url());
